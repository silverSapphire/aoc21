...
fn main() {
    ...
    for ball in balls {
        let mut winners = mark_boards(&ball, &mut boards, &mut sums);

        if boards.len() == 1 && winners.contains(&0) {
            println!("The last board to win has a score of {}", ball * sums[0]);
            break;
        }

        //Remove from largest indices first so we don't bump lower indices
        winners.sort();
        winners.reverse();
 ...
 */
fn mark_boards(ball: &u32, boards: &mut Vec<Vec<HashSet<u32>>>, sums: &mut Vec<u32>) -> Vec<usize> {
    ...
    for (board_i, board) in boards.iter_mut().enumerate() {
        ...
        //Winner
        if winner {
            //return (board_i, sums[board_i] as i32);
